		...
		while True:
			...
			for i in range(lines_per_sample):
				...
				if mod_stat[i] == 0:
					...
					valid = False
				mean_mag = abs(mean)  # NOTE: always positive...
				if i == 0:
					mean_sum += 2.0 ** mean_mag * (1 if mean >= 0 else -1)
				sample_stat[i][0] += 2.0 ** mean_mag * (1 if mean >= 0 else -1)
				sample_stat[i][1] += 2.0 ** var + (2.0 ** mean_mag) ** 2
				sample_stat[i][2] += 2.0 ** mag
			# a whole sample is read, update stat
			if valid:
				sample_count += 1
				stat += sample_stat
		mean_sum /= sample_count
		print(mean_sum, sample_stat[0][0] / sample_count)
		return (param_idx, total_levels, boot_levels, square_between_boots, mod_stat), stat, sample_count
...
def merge_stats(stat_list):
	...
	stat = np.log2(np.abs(np.hstack([stat, (stat[:, 1] - stat[:, 0]**2)[:, np.newaxis]])))
	return config, stat, sample_count


data_files = ["../cmake-build-release/set1/output_{}".format(i) for i in range(8)]
